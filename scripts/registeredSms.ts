...
const CACHE_FILE = 'registeredSms.json';
...
function persist(data: StoreShape): void {
  try {
    ...
    console.warn('[registeredSms] 저장 실패', e);
  }
}
...
export function isSmsRegistered(smsId: string): boolean {
  if (!smsId) return false;
  return smsId in load();
}

export function markSmsRegistered(smsId: string): void {
  if (!smsId) return;
  const data = load();
  data[smsId] = Date.now();
...
export function clearRegisteredSms(): void {
  memory = {};
  persist(memory);
}
